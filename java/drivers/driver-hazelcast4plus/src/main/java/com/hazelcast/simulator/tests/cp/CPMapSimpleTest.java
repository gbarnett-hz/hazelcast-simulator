...
public class CPMapSimpleTest extends HazelcastTest {
	...
	public void prepare() {
		...
		List<Future<?>> futures = new ArrayList<>(keys);
		try {
			for (long key = 0; key < keys; key++) {
				final long mapKey = key;
				final byte[] value = values[random.nextInt(values.length)];
				futures.add(executor.submit(() -> map.set(mapKey, value)));
			}
			for (Future<?> future : futures) {
				try {
					future.get();
				} catch (ExecutionException e) {
					throw new RuntimeException("Failed to populate CPMap during prepare",
							e.getCause());
				}
			}
		} catch (InterruptedException e) {
			Thread.currentThread().interrupt();
			throw new RuntimeException("Prepare interrupted while populating CPMap", e);
		} finally {
			executor.shutdown();
			try {
